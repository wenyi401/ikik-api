...
- 合并上游 sub2api v0.2.4（519 个提交 / 876 个文件），前端与后端版本统一为 1.0.4。
- 新增 OpenCode 平台接入：账号模式、模型分组与额度窗口对接，默认使用官方 CLI 的 TLS 指纹。
- 修复合并过程中丢失的上游能力：自定义 EasyPay 付款方式在收银台被整体隐藏、腾讯/阿里云人机验证下邮箱补全流程（发验证码与提交）被静默中止、账号用量行的 Ollama 用量与配额重置结果无法回传、用量刷新时模型统计被清空、导入无效 JSON 不提示文件名等。
- 管理后台恢复紧凑首页开关、默认平台配额矩阵、充值返利开关、Claude OAuth 系统提示词注入等设置项。
- 前端全量测试对比合并前基线新增失败 0；Go 后端构建与既有测试基线保持一致。
