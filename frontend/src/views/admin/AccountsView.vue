<template>
  <AppLayout>
          ...
          >
            <template #after>
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                <UiIconButton
                  :label="t('admin.accounts.moreActions')"
                ...
                <Teleport to="body">
                  ...
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="px-2 py-2 text-xs font-semibold uppercase text-gray-400">
                      ...
                      <button class="account-tools-menu-item" @click="openCredentialImport">
                        <span class="account-tools-menu-icon bg-cyan-50 text-cyan-600 dark:bg-cyan-900/30 dark:text-cyan-300">
                          <Icon name="upload" size="sm" />
                        </span>
                        <span>{{ t('admin.accounts.credentialImport') }}</span>
                      </button>
                      ...
                      <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                        ...
                        <span v-if="selIds.length" class="rounded-full bg-primary-100 px-2 py-0.5 text-xs text-primary-700">
                      ...
                      <div class="my-2 border-t border-gray-100 dark:border-dark-700" />
                      <div class="flex items-center justify-between px-2 py-2 text-xs font-semibold uppercase text-gray-400">
        ...
        >
          ...
          <template #cell-proxy="{ row }">
            <div v-if="row.proxy" class="flex flex-col items-start gap-1">
              ...
              <div v-if="row.proxy.expires_at" class="flex items-center gap-2 text-xs">
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                <span
                  class="inline-flex max-w-[180px] items-center rounded-md bg-stone-100 px-1.5 py-0.5 text-[11px] font-medium text-stone-700 dark:bg-stone-800 dark:text-stone-200"
                  :title="t('admin.accounts.proxyFallbackOrigin', { name: row.proxy_fallback_origin_name || row.proxy_fallback_origin_id })"
                >
                  {{ t('admin.accounts.proxyFallbackBadge') }}
                </span>
                <button
                  ...
                  @click.stop="onRevertFallback(row)"
                >
    ...
    <CreateAccountModal :show="showCreate" :proxies="proxies" :groups="assignableAccountGroups" @close="showCreate = false" @created="reload" />
    <EditAccountModal
      :show="showEdit"
      :account="edAcc"
      :proxies="proxies"
      :groups="assignableAccountGroups"
      account-scope="admin"
      :allow-proxy="true"
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :anchor-rect="menu.pos as unknown as DOMRect | null" @close="menu.show = false" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" @revert-proxy-fallback="handleRevertProxyFallback" />
    ...
    <ConfirmDialog :show="showCreateShadowDialog" :title="t('admin.accounts.createSparkShadow')" :message="t('admin.accounts.createSparkShadowConfirm', { name: creatingShadowAcc?.name })" @confirm="confirmCreateSparkShadow" @cancel="cancelCreateSparkShadow" />
...
import { accountAssignableGroups as filterAccountAssignableGroups } from '@/utils/accountGroups'
...
import type { Account, AccountPlatform, AccountType, AccountSchedulerGroupScore, AccountUsageInfo, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSnapshot } from '@/types'
...
const assignableAccountGroups = computed(() => filterAccountAssignableGroups(groups.value))
...
// Account tools and column settings
...
const updateAccountToolsDropdownPosition = () => {
  const trigger = accountToolsDropdownRef.value
  if (!trigger) return
  Object.assign(
...
const openCredentialImport = () => {
  closeAccountToolsDropdown()
  showCredentialImport.value = true
}
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `ikik-api-account-${timestamp}.json`
...
const handleRevertProxyFallback = async (a: Account) => {
  try {
    const updated = await adminAPI.accounts.revertProxyFallback(a.id)
    patchAccountInList(updated)
    enterAutoRefreshSilentWindow()
    appStore.showSuccess(t('admin.accounts.revertProxyFallbackSuccess'))
  } catch (error: any) {
    console.error('Failed to revert proxy fallback:', error)
    appStore.showError(error?.response?.data?.message || error?.message || t('admin.accounts.revertProxyFallbackFailed'))
  }
}
// The table-level fallback action uses the same update path as the account menu.
const onRevertFallback = (a: Account) => handleRevertProxyFallback(a)
...
const proxyExpiryBadge = (p: AccountProxy): string => proxyExpiryBadgeClass(p.expires_at ?? null, p.status)
const proxyExpiryText = (p: AccountProxy): string => {
  const { key, params } = proxyExpiryLabelKey(p.expires_at ?? null, p.status)
