...
const reasoningEffortValuesForPlatform = (platform: GroupPlatform): readonly string[] =>
  supportsReasoningEffortPolicyPlatform(platform) ? openAIReasoningEffortValues : []
...
export function normalizeReasoningEffortForPlatform(
  ...
  return reasoningEffortValuesForPlatform(platform).some((allowed) => allowed === normalized)
    ? normalized
    : ''
}
...
export function reasoningEffortMappingsToRows(
  ...
  return (mappings ?? []).flatMap((mapping) => {
    ...
    return from && to ? [createReasoningEffortMappingRow({ from, to })] : []
  })
}
...
export function reasoningEffortMappingsToAPI(
  ...
  return rows.map((row) => ({ from: row.from.trim(), to: row.to.trim() }))
}
