      ...
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          <div class="h-10 w-10 overflow-hidden rounded-xl shadow-md">
            <img :src="siteLogo || '/ik-logo.svg'" alt="Logo" class="h-full w-full object-contain" />
          </div>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div v-if="showResults">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          >
            <div class="flex flex-col gap-3 border-b border-gray-200 px-8 py-5 dark:border-dark-700 sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400">
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">{{ row.date }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-gray-700 dark:text-dark-200">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-gray-700 dark:text-dark-200">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-gray-700 dark:text-dark-200">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-gray-700 dark:text-dark-200">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-gray-700 dark:text-dark-200">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm font-medium tabular-nums text-gray-900 dark:text-white">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
