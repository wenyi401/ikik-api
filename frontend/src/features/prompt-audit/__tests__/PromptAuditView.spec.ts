...
const baseConfig = (): PromptAuditConfig => ({
  enabled: true, blocking_enabled: false, blocking_latest_turn_only: false, async_latest_user_only: true, enforcement_mode: 'shadow', store_pass_events: false, effective_mode: 'async_audit', strategy: 'priority',
...
describe('PromptAuditView', () => {
  ...
  it('requires confirmation for blocking and disables it when audit is turned off', async () => {
    mocks.getConfig.mockResolvedValue({ ...baseConfig(), enforcement_mode: 'enforce' })
    const wrapper = mountView()
    await flushPromises()
    await wrapper.get('[data-test="tab-config"]').trigger('click')
