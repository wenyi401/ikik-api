...
export async function updateCompositeRoute(
  ...
  const { data } = await apiClient.put<CompositeModelRoute>(`/admin/groups/${id}/composite-routes/${routeId}`, route)
  return data
}

export async function deleteCompositeRoute(id: number, routeId: number): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(`/admin/groups/${id}/composite-routes/${routeId}`)
  return data
}
...
export async function previewCompositeRoute(
  ...
  const { data } = await apiClient.post<CompositeRouteDecision>(`/admin/groups/${id}/composite-routes/preview`, request)
  return data
}
