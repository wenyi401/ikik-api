...
export default {
  ...
  admin: {
    riskControl: {
      modeAdaptive: '智能异步',
      modeAdaptiveDesc: '请求立即放行，后台按用户风险和审核阶段动态抽检',
      providerModelClassifier: '模型分类器',
      adaptive: {
        title: '智能异步策略',
        summary: '新用户全量审核，稳定用户自动降低抽检率，持续风险行为逐级提高审核强度',
        enforcementMode: '处置方式',
        shadow: '影子',
        shadowDesc: '只记录和评分',
        notify: '通知',
        notifyDesc: '评分并按冷却时间通知',
        enforce: '执行',
        enforceDesc: '临界风险时停用触发的密钥',
        auditStages: '审核阶段',
        fullAuditRequests: '全量审核次数',
        rampAuditRequests: '过渡阶段结束次数',
        rampSampleRate: '过渡抽检率 (%)',
        trustedSampleRate: '可信抽检率 (%)',
        watchSampleRate: '观察抽检率 (%)',
        highRiskSampleRate: '高风险抽检率 (%)',
        dailyDecay: '每日衰减 (%)',
        notificationCooldown: '通知冷却 (小时)',
        scoreWeights: '风险权重',
        scoreThresholds: '风险分层',
        low: '低',
        medium: '中',
        severe: '严重',
        invalidAuditStages: '过渡阶段结束次数不能小于全量审核次数',
        invalidThresholds: '观察、高风险和临界阈值必须依次增大'
      },
      riskLevels: {
        new: '新用户',
        normal: '过渡',
      ...
      classifierPromptCustom: '自定义',
      classifierPromptContract: '输出字段与分类枚举不可修改',
      classifierPromptRequired: '自定义分类策略不能为空',
      ...
      groupPenalty: {
        title: '分组处罚',
        hint: '开启后，所选分类的审核分数达到独立阈值时立即处罚目标分组；管理员账号始终豁免。',
        firstBlockHours: '首次封禁（小时）',
        secondBlockHours: '第二次封禁（小时）',
        thirdBlock: '第三次处罚',
        permanent: '永久封禁该分组',
        targetGroups: '处罚目标分组',
        targetGroupsHint: '命中规则后封禁用户使用这里选中的分组，不影响其他分组。',
        searchGroups: '搜索分组名称、平台或 ID',
        categories: '处罚分类与独立阈值',
        categoriesHint: '勾选参与处罚的分类；每个分类按自己的审核分数阈值判断。',
        hitThreshold: '处罚阈值',
        targetGroupsRequired: '开启分组处罚后至少选择一个目标分组',
        categoriesRequired: '开启分组处罚后至少选择一个处罚分类',
        invalidDurations: '处罚时长必须为 1-8760 的整数，且第二次不能短于第一次',
        invalidThresholds: '每个已选分类的处罚阈值必须在 0%-100% 之间'
      },
      tabs: {
        adaptive: '智能策略',
        basic: '基础',
    ...
    accounts: {
      ...
      accountLevel: {
        ...
        userManualHint: '账号等级由系统根据账号凭证自动识别。',
      ...
      credentialImportWarning: '默认只接收官方 OAuth 凭证。导入 Claude Web 会话文件时，请先开启 Claude Web 模式。API Key、URL 和 Upstream 仍会被拒绝。',
      importClaudeWebMode: '按 Claude Web 格式解析',
      importClaudeWebModeHint: '用于导入包含 cookies、org_uuid 和账号信息的 Claude Web JSON',
      importClaudeWebFullCookie: '完整 Cookie',
      importClaudeWebFullCookieHint: '仅在导入的是完整浏览器 Cookie 时开启',
      ...
      dataImportIgnoredFiles: '已忽略 {count} 个不支持的文件',
      ...
      headerOverride: {
        title: '请求头覆写',
        hint: '转发请求时覆写同名请求头',
        info: '仅作用于该账号的出站请求。认证头和连接控制类请求头禁止覆写。',
        namePlaceholder: '请求头名称',
        valuePlaceholder: '覆写值',
        addRow: '添加请求头',
        fillTemplate: '填入模板',
        emptyValueHint: '值为空的行只作为占位，不会覆写请求头。',
        bulkDisableHint: '保存后会关闭所选账号的请求头覆写，并清空已有配置。',
        bulkReplaceHint: '保存后会替换所选账号已有的请求头覆写配置。',
        bulkEmptyRows: '保存前请至少添加一个请求头，或关闭开关以清空已有配置。',
        invalidName: '请求头名称无效',
        blockedName: '该请求头禁止覆写',
        duplicateName: '请求头名称重复',
        invalidValue: '请求头值无效',
    ...
    settings: {
      ...
      site: {
        ...
        homeContentIframeWarning: 'iframe 模式提示：部分网站设置了 X-Frame-Options 或 CSP 安全策略，禁止被嵌入到 iframe 中。如果页面显示空白或报错，请确认目标网站允许被嵌入，或考虑使用 HTML 模式自行构建页面内容。',
      ...
      purchase: {
        ...
        iframeWarning: 'iframe 提示：部分网站会通过 X-Frame-Options 或 CSP（frame-ancestors）禁止被 iframe 嵌入，出现空白时可引导用户使用”新窗口打开”。',
      ...
      openaiFastPolicy: {
        ...
        actionBlock: '拦截（拒绝请求）',
        actionForcePriority: '强制 priority',
        scope: '生效范围',
        scopeAll: '全部账号',
        scopeOAuth: '仅 OAuth 账号',
        scopeAPIKey: '仅 API Key 账号',
  ...
  onboarding: {
    ...
    admin: {
      ...
      accountPlatform: {
        title: '2. 选择平台',
        description: '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">选择该账号对应的服务商平台。</p><p style="padding: 8px 12px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 4px; font-size: 13px;"><b>重要：</b>平台必须与刚才创建的分组平台一致</p></div>',
        nextBtn: '下一步'
      },
      accountType: {
        title: '3. 授权方式',
