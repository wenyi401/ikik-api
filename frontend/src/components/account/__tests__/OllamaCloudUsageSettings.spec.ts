...
vi.mock('@/api/accounts', () => ({ accountsAPI: userApi }))
...
describe('OllamaCloudUsageSettings', () => {
  ...
  it('uses user-owned endpoints when rendered in user account scope', async () => {
    userApi.refreshOllamaCloudUsage.mockResolvedValueOnce(detailedState('pro'))
    const wrapper = mount(OllamaCloudUsageSettings, {
      props: { account: account(detailedState()), accountScope: 'user' }
    })
    await flushPromises()
    ...
    expect(userApi.refreshOllamaCloudUsage).toHaveBeenCalledWith(7)
    expect(api.refreshOllamaCloudUsage).not.toHaveBeenCalled()
  })
