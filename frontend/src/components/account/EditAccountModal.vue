    ...
    >
      ...
      <div v-if="isUserScope">
        ...
        <div class="grid grid-cols-2 gap-2">
          <button
            ...
            @click="setUserShareMode('private')"
          >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        ...
        <div v-if="isCNApiKeyAccount && account.platform !== 'opencode_go'">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            ...
            >
              {{ t(`admin.accounts.cnProviders.accountMode.${option.labelKey}`) }}
            </button>
          </div>
        ...
        <div v-if="account.platform !== 'antigravity'" class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <template v-else>
            ...
            <div v-if="modelRestrictionMode === 'whitelist'">
              <ModelWhitelistSelector v-model="allowedModels" :platform="account?.platform || 'anthropic'" :account-scope="accountScope" />
      ...
      <div v-if="!isUserScope && account.type === 'bedrock'" class="space-y-4">
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <!-- Whitelist Mode -->
          <div v-if="modelRestrictionMode === 'whitelist'">
            <ModelWhitelistSelector v-model="allowedModels" platform="anthropic" :account-scope="accountScope" />
            <p class="text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.selectedModels', { count: allowedModels.length }) }}
      ...
      <div
        v-if="(account as any)?.platform === 'anthropic' && ((account as any)?.type === 'oauth' || (account as any)?.type === 'setup-token')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4"
      >
        <div class="mb-3">
          <h3 class="input-label mb-0 text-base font-semibold">{{ t('admin.accounts.quotaControl.title') }}</h3>
      ...
      <div v-if="!isUserScope" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div v-if="(account as any)?.platform === 'antigravity'" class="flex items-center gap-2">
        ...
        <div v-if="(account as any)?.platform === 'antigravity'" class="mt-3 flex items-center gap-2">
...
const groupSelectorPlatform = computed<GroupPlatform | undefined>(() => {
  ...
  return props.account.platform === 'custom' ? customProtocolGroupPlatform.value : props.account.platform
})
...
watch(editApiProtocol, (protocol, previousProtocol) => {
  ...
  if (protocol === 'adaptive') {
    ...
    for (const item of editAdaptiveProtocolOptions.value) {
      if (!editAdaptiveBaseUrls.value[item.value]) {
        editAdaptiveBaseUrls.value[item.value] = defaults[item.value]
      }
    }
...
const onCnPresetSelect = (preset: { mode: CnAccountMode; protocol: CnApiProtocol; url: string }) => {
...
const apiKeySecretPlaceholder = computed(() => {
  ...
  if (props.account?.platform === 'zhipu') return '<api-key>.<secret>'
  if (props.account?.platform === 'kimi' || props.account?.platform === 'deepseek') return 'sk-...'
  return 'sk-ant-...'
})
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  form.concurrency = Math.max(1, newAccount.concurrency || PERSONAL_ACCOUNT_DEFAULT_CONCURRENCY)
  ...
  form.status = (newAccount.status === 'active' || newAccount.status === 'inactive' || newAccount.status === 'disabled' || newAccount.status === 'error')
    ? newAccount.status
    : 'active'
  form.group_ids = isUserScope.value ? [] : (newAccount.group_ids || [])
  ...
  autoResetCredit5hThreshold.value =
    typeof extra?.auto_reset_credit_5h_threshold === 'number'
      ? extra.auto_reset_credit_5h_threshold * 100
      : 100
  autoResetCredit7dThreshold.value =
    typeof extra?.auto_reset_credit_7d_threshold === 'number'
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    if (isUserScope.value) {
      ...
      if (newAccount.type === 'oauth') {
        ...
        editPlanType.value = readPlanType(newAccount.credentials as Record<string, unknown> | undefined)
      }
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (isCNProviderPlatform(newAccount.platform) || newAccount.platform === 'opencode_go') {
      ...
      const adaptiveDefaults = defaultCNAdaptiveBaseUrls(isCNProviderPlatform(newAccount.platform) ? newAccount.platform : 'opencode_go', currentOpenCodeOrCNMode())
    ...
    const platformDefaultUrl =
      newAccount.platform === 'openai'
        ...
        : newAccount.platform === 'gemini'
          ...
          : newAccount.platform === 'grok'
            ? 'https://api.x.ai/v1'
            : newAccount.platform === 'kiro' || newAccount.platform === 'custom'
              ? ''
...
const sanitizeUpdatePayload = (payload: Record<string, unknown>) => {
  ...
  if (isUserScope.value) {
    ...
    if (next.share_mode === 'public') {
      ...
      const concurrency = Number(next.concurrency)
      next.concurrency = Number.isFinite(concurrency) && concurrency > 0
        ? Math.floor(concurrency)
        : Math.max(1, props.account?.concurrency || PERSONAL_ACCOUNT_DEFAULT_CONCURRENCY)
    }
    next.load_factor = 0
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isUserScope.value) {
      updatePayload.concurrency = form.share_mode === 'public'
        ? PERSONAL_ACCOUNT_DEFAULT_CONCURRENCY
    ...
    if (props.account.type === 'apikey') {
      ...
      if (props.account.platform === 'custom' && !editBaseUrl.value.trim()) {
    ...
    // For Custom API Key accounts, keep the selected upstream protocol in extra.
    if (props.account.platform === 'custom' && props.account.type === 'apikey') {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
      updatePayload.extra = {
        ...currentExtra,
