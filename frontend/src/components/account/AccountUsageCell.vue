<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <!-- Grok OAuth accounts: passive xAI quota headers + local usage -->
...
const emit = defineEmits<{
  ...
  (e: 'usage-loaded', usage: AccountUsageInfo): void
}>()
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  _usageCache.set(usageCacheKey.value, { data: merged, ts: Date.now() })
}
