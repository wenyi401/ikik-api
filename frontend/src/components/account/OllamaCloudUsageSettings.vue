...
const usageAPI = computed(() => props.accountScope === 'user' ? accountsAPI : adminAPI.accounts)
...
const load = async () => {
  loading.value = true
  try {
    applyState(await usageAPI.value.getOllamaCloudUsage(props.account.id))
  } catch (error) {
...
const saveSession = async () => {
  ...
  saving.value = true
  try {
    applyState(await usageAPI.value.saveOllamaCloudUsageSession(props.account.id, session.value))
    session.value = ''
...
const deleteSession = async () => {
  ...
  try {
    applyState(await usageAPI.value.deleteOllamaCloudUsageSession(props.account.id))
    session.value = ''
...
const setAutoRefresh = async (enabled: boolean) => {
  saving.value = true
  try {
    applyState(await usageAPI.value.setOllamaCloudUsageAutoRefresh(props.account.id, enabled))
  } catch (error) {
...
const refreshUsage = async () => {
  ...
  try {
    applyState(await usageAPI.value.refreshOllamaCloudUsage(props.account.id))
