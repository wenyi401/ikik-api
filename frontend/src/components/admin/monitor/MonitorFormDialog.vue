<template>
  ...
  >
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div
        v-if="form.provider === PROVIDER_OPENAI && usesProbePart"
      ...
      >
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <button
            ...
            :key="option.value"
            type="button"
            :aria-pressed="form.api_mode === option.value"
            class="rounded-lg border-2 px-3 py-2 text-left transition-colors"
            :class="apiModeButtonClass(option.value)"
            @click="form.api_mode = option.value"
          >
            <span class="block text-sm font-semibold">{{ option.label }}</span>
            <span class="mt-0.5 block text-xs opacity-80">{{ option.hint }}</span>
          </button>
        </div>
      </div>
...
const templateOptions = computed(() => {
  const items = templatesCache.value.filter((template) => {
    if (template.provider !== form.provider) return false
    if (form.provider !== PROVIDER_OPENAI) return true
    return normalizeAPIMode(template.api_mode) === form.api_mode
  })
  return [
    { value: '', label: t('admin.channelMonitor.templateField.none') },
    ...items.map((template) => ({ value: String(template.id), label: templateOptionLabel(template) })),
  ]
})
...
const normalizeAPIMode = (mode: APIMode | undefined | null): APIMode =>
  mode === API_MODE_RESPONSES ? API_MODE_RESPONSES : API_MODE_CHAT_COMPLETIONS

const apiModeButtonClass = (mode: APIMode): string => {
  if (form.api_mode === mode) {
...
const templateOptionLabel = (template: ChannelMonitorTemplate): string => {
  if (template.provider !== PROVIDER_OPENAI) return template.name
  const labelKey = normalizeAPIMode(template.api_mode) === API_MODE_RESPONSES
    ? 'admin.channelMonitor.form.apiModeResponses'
    : 'admin.channelMonitor.form.apiModeChatCompletions'
  return `${template.name} · ${t(labelKey)}`
}

const clearRequestSnapshot = () => {
  form.template_id = null
  form.extra_headers = {}
