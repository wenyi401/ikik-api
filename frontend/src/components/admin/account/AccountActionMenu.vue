      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="hasProxyFallback" @click="$emit('revert-proxy-fallback', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-stone-700 hover:bg-gray-100 dark:text-stone-200 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" />
              {{ t('admin.accounts.revertProxyFallback') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'revert-proxy-fallback'])
const { t } = useI18n()
