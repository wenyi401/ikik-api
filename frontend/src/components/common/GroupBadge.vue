<template>
  ...
  >
    ...
    <span
      v-if="isPrivateScope"
      :class="[scopeIconClass, 'shrink-0']"
      :title="t('groups.private')"
      :aria-label="t('groups.private')"
    >
      <Icon name="lock" size="xs" :stroke-width="2" />
    </span>
    <!-- Right side label -->
    <span v-if="showLabel" :class="[labelClass, 'shrink-0']">
...
const labelClass = computed(() => {
  ...
  if (String(props.platform) === 'composite') {
...
const scopeIconClass = computed(() => {
  if (isPrivateScope.value) {
    return 'inline-flex h-4 w-4 items-center justify-center rounded bg-slate-200/70 text-slate-700 dark:bg-slate-700 dark:text-slate-200'
  }
  return 'inline-flex h-4 w-4 items-center justify-center rounded bg-red-100/80 text-red-700 dark:bg-red-900/30 dark:text-red-300'
})
