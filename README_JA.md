...
サイト：[https://ikik.net](https://ikik.net)
...
- 上流 sub2api v0.2.4（519 コミット / 876 ファイル）をマージし、フロントエンドとバックエンドのバージョンを 1.0.4 に統一。
- OpenCode プラットフォーム対応を追加（アカウントモード、モデルグループ、利用枠ウィンドウ連携。既定で公式 CLI の TLS フィンガープリントを使用）。
- マージで失われていた上流機能を修復：カスタム EasyPay 決済手段が決済画面で非表示、Tencent/Aliyun のキャプチャ有効時に確認コード送信と送信が中断、アカウント利用行の Ollama 利用量とクォータリセット結果が反映されない、更新時にモデル統計が空になる、無効な JSON 取り込みでファイル名が示されない。
- 管理画面のコンパクトホーム切り替え、既定プラットフォームクォータ表、管理者チャージ返金スイッチ、Claude OAuth システムプロンプト注入などを復元。
- フロントエンド全テストはマージ前ベースライン比で新規失敗 0。Go バックエンドのビルドと既存テスト基準も維持。
