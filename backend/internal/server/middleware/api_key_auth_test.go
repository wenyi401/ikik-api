...
func TestAPIKeyAuthRejectsExclusiveGroupWhenUserNoLongerAllowed(t *testing.T) {
	...
	groupID := int64(101)
	newRouter := func(allowedGroups []int64) *gin.Engine {
		group := &service.Group{
			ID:          groupID,
			Name:        "exclusive",
			Status:      service.StatusActive,
		...
		cfg := &config.Config{RunMode: config.RunModeSimple}
		apiKeyService := service.NewAPIKeyService(apiKeyRepo, nil, nil, nil, nil, nil, cfg)
		return newAuthTestRouter(apiKeyService, nil, cfg)
	}

	t.Run("allows_user_with_exclusive_group_permission", func(t *testing.T) {
		router := newRouter([]int64{groupID})
	...
	t.Run("rejects_old_key_after_permission_removed", func(t *testing.T) {
