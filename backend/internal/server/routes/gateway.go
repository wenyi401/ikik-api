...
func RegisterGatewayRoutes(
	...
	// ikik：同上，但额外挂 Pet GatewayActivity 活动埋点（生成类动作路由）。
	activityRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, h.Pet.GatewayActivity(), compositeTarget, requireGroupAnthropic, handler)
	}
	activityRoute(http.MethodPost, "/responses", bodyLimit, responsesHandler)
	activityRoute(http.MethodPost, "/responses/*subpath", bodyLimit, guardResponsesSubpath(responsesHandler))
	activityRoute(http.MethodPost, "/alpha/search", textBodyLimit, h.OpenAIGateway.AlphaSearch)
	activityRoute(http.MethodGet, "/responses", bodyLimit, func(c *gin.Context) {
		h.OpenAIGateway.ResponsesWebSocket(c)
	})
	rootRoute(http.MethodGet, "/models", bodyLimit, modelsHandler)
	rootRoute(http.MethodPost, "/messages/count_tokens", bodyLimit, countTokensHandler)
	codexDirect := r.Group("/backend-api/codex")
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, h.Pet.GatewayActivity(), compositeTarget, requireGroupAnthropic)
	{
	...
	activityRoute(http.MethodPost, "/chat/completions", bodyLimit, func(c *gin.Context) {
		if isOpenAIResponsesCompatibleGatewayPlatform(c) {
			h.OpenAIGateway.ChatCompletions(c)
			return
		}
	...
	activityRoute(http.MethodPost, "/embeddings", textBodyLimit, func(c *gin.Context) {
		if !isOpenAIOnlyEndpointGatewayPlatform(c) {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{
				"error": gin.H{
	...
	activityRoute(http.MethodPost, "/images/generations", bodyLimit, imagesHandler)
	activityRoute(http.MethodPost, "/images/edits", bodyLimit, imagesHandler)
	activityRoute(http.MethodPost, "/images/generations/async", bodyLimit, h.AsyncImage.Submit)
	activityRoute(http.MethodPost, "/images/edits/async", bodyLimit, h.AsyncImage.Submit)
	rootRoute(http.MethodGet, "/images/tasks/:task_id", bodyLimit, h.AsyncImage.Get)
	activityRoute(http.MethodPost, "/videos", bodyLimit, videoGenerationHandler)
	activityRoute(http.MethodPost, "/videos/generations", bodyLimit, videoGenerationHandler)
	activityRoute(http.MethodPost, "/videos/edits", bodyLimit, videoEditHandler)
	activityRoute(http.MethodPost, "/videos/extensions", bodyLimit, videoExtensionHandler)
	...
	activityRoute(http.MethodPost, "/tts", bodyLimit, rootVoiceHandler("tts"))
	activityRoute(http.MethodPost, "/stt", bodyLimit, rootVoiceHandler("stt"))
	activityRoute(http.MethodPost, "/custom-voices", bodyLimit, rootVoiceHandler("custom-voices"))
	...
	activityRoute(http.MethodGet, "/realtime", bodyLimit, func(c *gin.Context) {
		if getGroupPlatform(c) != service.PlatformGrok {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Realtime API is not supported for this platform"}})
			return
	...
	activityRoute(http.MethodPost, "/web_search", bodyLimit, func(c *gin.Context) {
		if getGroupPlatform(c) != service.PlatformGrok {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Web Search API is not supported for this platform"}})
			return
	...
	activityRoute(http.MethodPost, "/x_search", bodyLimit, func(c *gin.Context) {
		if getGroupPlatform(c) != service.PlatformGrok {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "X Search API is not supported for this platform"}})
			return
