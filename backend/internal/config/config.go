...
type GatewayConfig struct {
	...
	// 这类工具由上游以「账号主人」身份在服务端执行，在共享 OAuth 账号场景下不应暴露给下游用户。
	CodexBlockConnectorTools bool `mapstructure:"codex_block_connector_tools"`
	// OpenAIImagesResponsesReasoningEffort controls OAuth image bridge reasoning.effort.
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.Gateway.OpenAIImagesResponsesReasoningEffort = NormalizeOpenAIImagesResponsesReasoningEffort(cfg.Gateway.OpenAIImagesResponsesReasoningEffort)
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "ikik-api")
	...
	viper.SetDefault("dashboard_cache.key_prefix", "ikik-api:")
