...
func TestPromptAuditRunsBeforeAnyPreFlightBlockReturn(t *testing.T) {
	tests := []promptAuditOrderCase{
	...
	for _, tt := range tests {
		t.Run(tt.file+"/"+tt.function, func(t *testing.T) {
			functionSource := stripGoComments(goFunctionSource(t, tt.file, tt.function))
			preFlightIndex := strings.Index(functionSource, "runPreFlightHooks(")
			auditIndex := strings.Index(functionSource, "checkSecurityAudit(")
			require.NotEqual(t, -1, preFlightIndex, "missing pre-flight gate")
			require.NotEqual(t, -1, auditIndex, "missing Prompt Audit gate")
			require.Less(t, preFlightIndex, auditIndex, "legacy pre-flight must finish before Prompt Audit")
			require.NotContains(t, functionSource[preFlightIndex:auditIndex], "return",
				"Prompt Audit must run even when legacy pre-flight blocks")
		})
	}
}
