...
func (s *stubAdminService) UpdateUserBalance(ctx context.Context, userID int64, balance float64, operation string, notes string, countAsRevenue bool) (*service.User, error) {
...
func (s *stubAdminService) GetAllGroupsIncludingInactive(ctx context.Context, scope string) ([]service.Group, error) {
	return s.groups, nil
}
...
func (s *stubAdminService) GetGroupModelsListCandidates(ctx context.Context, id int64, platform string) ([]string, error) {
	...
	models := make([]string, 0, len(s.groups))
...
func (s *stubAdminService) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID, proxyID int64, privacyMode string, sortBy, sortOrder string) ([]service.Account, int64, error) {
...
func (s *stubAdminService) RevertAccountProxyFallback(ctx context.Context, id int64) (*service.Account, error) {
	proxyID := int64(4)
	account := service.Account{ID: id, Name: "account", Status: service.StatusActive, ProxyID: &proxyID}
	return &account, nil
}
