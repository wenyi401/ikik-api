...
//   - api_key_group_id: filter users by API keys that can route to this group
...
func (h *UserHandler) UpdateBalance(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.users.balance.update", idempotencyPayload, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		user, execErr := h.adminService.UpdateUserBalance(ctx, userID, req.Balance, req.Operation, req.Notes, req.CountAsRevenue)
		if execErr != nil {
			return nil, execErr
		}
