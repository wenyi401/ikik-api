...
func runSecurityAudit(c *gin.Context, reqLog *zap.Logger, coordinator *securityaudit.Coordinator, legacy *service.ContentModerationService, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol, model string, body []byte, stage string) *securityaudit.Decision {
	...
	if coordinator == nil {
		if legacyModerationAlreadyChecked(c) {
			if prechecked := precheckedLegacyModerationDecision(c); prechecked != nil {
				return securityAuditDecisionFromLegacy(prechecked)
			}
			if cacheCompletion {
		...
		decision := securityAuditDecisionFromLegacy(legacy)
...
func buildSecurityAuditRequest(c *gin.Context, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol, model string, body []byte, stage string) securityaudit.Request {
	...
	request := securityaudit.Request{
		...
		LegacyAlreadyChecked: legacyModerationAlreadyChecked(c),
		PrecheckedLegacy:     precheckedLegacyModerationDecision(c),
	}
...
func legacyModerationAlreadyChecked(c *gin.Context) bool {
	if c == nil {
		return false
	}
	checked, exists := c.Get(legacyModerationCheckedContextKey)
	return exists && checked == true
}

func precheckedLegacyModerationDecision(c *gin.Context) *securityaudit.LegacyDecision {
	if c == nil {
		return nil
	}
	value, exists := c.Get(legacyModerationDecisionContextKey)
	if !exists {
		return nil
	}
	decision, ok := value.(*gatewayhook.Decision)
	if !ok || decision == nil || !decision.Blocked {
		return nil
	}
	return &securityaudit.LegacyDecision{
		Blocked:    true,
		Message:    decision.Message,
		StatusCode: preFlightStatus(decision),
		ErrorCode:  preFlightErrorCode(decision),
		Action:     "block",
	}
}

func securityAuditDecisionFromLegacy(legacy *securityaudit.LegacyDecision) *securityaudit.Decision {
	decision := &securityaudit.Decision{
		Kind: securityaudit.DecisionAllow, HTTPStatus: http.StatusOK, Legacy: legacy, AllowNextStage: true,
	}
	...
	code := strings.TrimSpace(legacy.ErrorCode)
	if code == "" {
