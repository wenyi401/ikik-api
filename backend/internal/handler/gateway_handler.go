...
func (h *GatewayHandler) codexModelIDsForGroup(ctx context.Context, group *service.Group, platformOverride string) []string {
	...
	if group.ModelAllowlistEnabled() {
		source := group.ModelAllowlist.FilterForListing(modelListingSource(platform, availableModels, fallbackModels))
		if group.CustomModelsListEnabled() {
			return filterModelsByCustomList(source, fallbackModels, group.ModelsListConfig.Models)
		}
		return source
	...
	if group.CustomModelsListEnabled() {
		return filterModelsByCustomList(
			modelListingSource(platform, availableModels, fallbackModels),
			fallbackModels,
...
func cloneAPIKeyWithGroup(apiKey *service.APIKey, group *service.Group) *service.APIKey {
	...
	if apiKey.User != nil && (apiKey.GroupID == nil || *apiKey.GroupID != group.ID) {
		...
		// The auth snapshot override belongs to the API key's resolved group.
		// A fallback group must resolve its own override during billing admission.
...
// submitMandatoryUsageRecordTask never drops billing work on a full worker pool.
