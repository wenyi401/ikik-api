...
func userRiskGroupBlocksFromService(blocks []service.UserRiskGroupBlock) []UserRiskGroupBlock {
	result := make([]UserRiskGroupBlock, 0, len(blocks))
	for _, block := range blocks {
		result = append(result, UserRiskGroupBlock{
			GroupID: block.GroupID, BlockedUntil: block.BlockedUntil, Permanent: block.Permanent,
		})
	}
	return result
}
...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		GroupID:                         k.GroupID,
		Status:                          k.Status,
		OpenAIExperimentalPromptEnabled: k.OpenAIExperimentalPromptEnabled,
