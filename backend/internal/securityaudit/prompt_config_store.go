...
func (m *ConfigManager) Public() (PublicConfig, error) {
	...
	return publicConfigWithTokenStatus(cloneStorageConfig(snapshot.storage), snapshot.active), nil
}
...
func (m *ConfigManager) observeExpectedState(raw string, riskControlEnabled bool) {
	...
	if intent.EnforcementMode == "" && intent.BlockingEnabled {
		intent.EnforcementMode = EnforcementEnforce
	}
