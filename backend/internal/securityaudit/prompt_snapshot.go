...
// when configured. Asynchronous auditing selects its own full or latest-user
// extractor in the enqueuer.
...
func extractPromptSnapshot(req Request, latestTurnOnly bool, latestUserOnly bool) (PromptSnapshot, error) {
	var document any
	...
	if latestUserOnly {
		segments = latestUserSegments(extracted)
	} else if latestTurnOnly {
...
func latestUserSegments(values []promptSegment) []string {
	...
	return promptSegmentTexts(normalized[start:end])
}
