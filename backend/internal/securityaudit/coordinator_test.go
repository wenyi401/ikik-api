...
func TestCoordinatorSkipsLegacyWhenPreFlightAlreadyCheckedIt(t *testing.T) {
	tests := []struct {
		name            string
		mode            Mode
		prompt          *PromptDecision
		...
		{name: "off", mode: ModeOff},
		{name: "async", mode: ModeAsync, wantEnqueues: 1},
		{name: "blocking", mode: ModeBlocking, prompt: &PromptDecision{Kind: DecisionAllow, AllowNextStage: true}, wantEvaluations: 1},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			legacy := &fakeLegacyEngine{decision: &LegacyDecision{Blocked: true}}
			prompt := &fakePromptEngine{mode: tt.mode, decision: tt.prompt}
			decision := NewCoordinator(legacy, prompt).Check(context.Background(), Request{
				LegacyAlreadyChecked: true,
			})
...
func TestCoordinatorReusesPrecheckedLegacyBlockAndStillRunsPrompt(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "async", mode: ModeAsync, wantEnqueues: 1},
		{name: "blocking", mode: ModeBlocking, wantEvaluations: 1},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			prechecked := &LegacyDecision{
				Blocked: true, StatusCode: http.StatusUnavailableForLegalReasons,
				ErrorCode: "legacy_exact", Message: "legacy block wins", Action: "block",
			}

			decision := NewCoordinator(legacy, prompt).Check(context.Background(), Request{
				LegacyAlreadyChecked: true,
				PrecheckedLegacy:     prechecked,
			})

			require.Equal(t, DecisionBlock, decision.Kind)
			require.Equal(t, http.StatusUnavailableForLegalReasons, decision.HTTPStatus)
			require.Equal(t, "legacy_exact", decision.ErrorCode)
			require.Equal(t, "legacy block wins", decision.ClientMessage)
			require.NotSame(t, prechecked, decision.Legacy)
			require.Equal(t, prechecked, decision.Legacy)
			require.Zero(t, legacy.calls.Load())
			require.Equal(t, tt.wantEnqueues, prompt.enqueues.Load())
			require.Equal(t, tt.wantEvaluations, prompt.evaluates.Load())
		})
