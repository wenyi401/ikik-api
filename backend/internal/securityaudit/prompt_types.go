...
type Request struct {
	...
	Body       []byte
	Stage      string
	// LegacyAlreadyChecked and PrecheckedLegacy are request-local coordination metadata.
	// They prevent duplicate legacy scans while preserving a pre-flight block as
	// the Coordinator's highest-priority decision. They are never persisted.
	LegacyAlreadyChecked bool
	PrecheckedLegacy     *LegacyDecision
}
...
func (r Request) Clone() Request {
	...
	if r.PrecheckedLegacy != nil {
		decision := *r.PrecheckedLegacy
		r.PrecheckedLegacy = &decision
	}
	return r
}
