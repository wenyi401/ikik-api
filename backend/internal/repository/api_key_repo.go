...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	return r.createAPIKeyWithGroupRoutes(ctx, key)
}
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldName,
			apikey.FieldStatus,
			apikey.FieldOpenaiExperimentalPromptEnabled,
		...
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				...
				user.FieldRpmLimit,
				user.FieldOpenaiExperimentalPromptUnlocked,
			)
			q.WithAllowedGroups(func(gq *dbent.GroupQuery) {
				gq.Select(group.FieldID)
			})
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldOpenaiExperimentalPromptEnabled,
				group.FieldRpmLimit,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	return r.updateAPIKeyWithGroupRoutes(ctx, key, fields)
}
...
func (r *apiKeyRepository) ClearGroupIDByGroupID(ctx context.Context, groupID int64) (int64, error) {
	return r.clearAPIKeyGroupBindings(ctx, groupID)
}
...
func (r *apiKeyRepository) UpdateGroupIDByUserAndGroup(ctx context.Context, userID, oldGroupID, newGroupID int64) (int64, error) {
	return r.moveAPIKeyGroupBindings(ctx, userID, oldGroupID, newGroupID)
}
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		Name:                            m.Name,
		Status:                          m.Status,
		OpenAIExperimentalPromptEnabled: m.OpenaiExperimentalPromptEnabled,
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		Status:                           u.Status,
		DeveloperAPIEnabled:              u.DeveloperAPIEnabled,
		OpenAIExperimentalPromptUnlocked: u.OpenaiExperimentalPromptUnlocked,
		OnboardingMode:                   string(u.OnboardingMode),
		ShareCardText:                    u.ShareCardText,
		ShareCardTextColor:               u.ShareCardTextColor,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		OpenAIExperimentalPromptEnabled: g.OpenaiExperimentalPromptEnabled,
		RPMLimit:                        g.RpmLimit,
