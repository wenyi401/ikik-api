...
func (s *GroupRepoSuite) TestListWithFiltersByScope_ExcludesUserPrivateGroups() {
	publicGroup := &service.Group{
		Name:             "scope-filter-public",
		Platform:         service.PlatformOpenAI,
	...
	privateGroup := &service.Group{
		Name:             "scope-filter-private",
		Platform:         service.PlatformOpenAI,
	...
	s.Require().NoError(s.repo.Create(s.ctx, publicGroup))
	s.Require().NoError(s.repo.Create(s.ctx, privateGroup))

	groups, page, err := s.repo.ListWithFiltersByScope(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 10},
		service.PlatformOpenAI,
		"",
	...
	s.Equal(publicGroup.ID, groups[0].ID)
	s.Equal(service.GroupScopePublic, groups[0].Scope)
}
