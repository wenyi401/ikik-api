...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs\\s+WHERE \\(request_type = \\$1 OR \\(request_type = 0 AND stream = FALSE AND openai_ws_mode = FALSE\\)\\)").
		...
		}).AddRow(int64(1), int64(2), int64(3), int64(4), int64(2), int64(2), 1.2, 1.0, 1.2, 20.0, 0.0, int64(0)))
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs").
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"avg_first_token_ms", "requests_with_first_token",
		}).AddRow(int64(50), int64(1000), int64(2000), int64(100), int64(60), int64(40), 15.0, 12.5, 11.0, 100.0, 0.0, int64(0)))
...
func TestBuildRequestTypeFilterConditionWithAlias(t *testing.T) {
	where, args := buildRequestTypeFilterConditionWithAlias(4, int16(service.RequestTypeStream), "ul")

	require.Equal(t, "(ul.request_type = $4 OR (ul.request_type = 0 AND ul.stream = TRUE AND ul.openai_ws_mode = FALSE))", where)
