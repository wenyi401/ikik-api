...
func TestAuthCacheInvalidationMigration_SecurityCoverageAndNoPlaintextPayload(t *testing.T) {
	content, err := migrations.FS.ReadFile("203_auth_cache_invalidation_outbox.sql")
	require.NoError(t, err)
	...
	for _, required := range []string{
		...
		"REFERENCING NEW TABLE AS new_routes", "REFERENCING OLD TABLE AS old_routes",
		"api_key_group_routes AS r", "SELECT DISTINCT",
		"o.api_key_id IS DISTINCT FROM n.api_key_id",
		"o.group_id IS DISTINCT FROM n.group_id",
		"o.priority IS DISTINCT FROM n.priority",
		"o.weight IS DISTINCT FROM n.weight",
		"o.enabled IS DISTINCT FROM n.enabled",
		"o.cooldown_seconds IS DISTINCT FROM n.cooldown_seconds",
	...
	require.GreaterOrEqual(t, strings.Count(sqlText, "SELECT DISTINCT"), 6)
