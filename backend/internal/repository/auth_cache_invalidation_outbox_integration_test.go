...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	secondaryGroup := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name: fmt.Sprintf("auth-outbox-secondary-group-%d", suffix), RateMultiplier: 1, IsExclusive: true,
	})
	...
	t.Cleanup(func() {
		...
		_, err := integrationDB.ExecContext(ctx, "DELETE FROM user_allowed_groups WHERE user_id = $1 OR group_id IN ($2, $3)", user.ID, group.ID, secondaryGroup.ID)
		require.NoError(t, err)
	...
	_, err := integrationDB.ExecContext(ctx, `
		INSERT INTO api_key_group_routes
			(api_key_id, group_id, priority, weight, enabled, cooldown_seconds)
		VALUES
			($1, $2, 100, 1, TRUE, 30),
			($1, $3, 200, 1, TRUE, 30)`,
		key.ID, group.ID, secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "multi-route insert must enqueue one invalidation per key")
	clear()

	_, err = integrationDB.ExecContext(ctx,
		"UPDATE api_key_group_routes SET updated_at = NOW() WHERE api_key_id = $1", key.ID)
	require.NoError(t, err)
	require.Zero(t, count(), "route timestamp-only updates must not enqueue")

	_, err = integrationDB.ExecContext(ctx,
		"UPDATE api_key_group_routes SET priority = priority + 1 WHERE api_key_id = $1", key.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "multi-route policy update must enqueue one invalidation per key")
	clear()
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary route group disable must enqueue bound keys")
	clear()
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary-route exclusive-group revocation must enqueue")
	clear()

	_, err = integrationDB.ExecContext(ctx, "DELETE FROM api_key_group_routes WHERE api_key_id = $1", key.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "multi-route delete must enqueue one invalidation per key")
	clear()
