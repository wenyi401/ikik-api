...
func TestAdminService_GroupRequiredAccountLevelTeamRoundTrip(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.NoError(t, err)
	require.Equal(t, AccountLevelTeam, created.RequiredAccountLevel)
	require.Equal(t, AccountLevelTeam, repo.created.RequiredAccountLevel)
...
func TestNormalizeAdminGroupRequiredAccountLevelAcceptsEverySharedPoolLevel(t *testing.T) {
	for _, level := range []string{
		...
		t.Run(level, func(t *testing.T) {
			got, err := normalizeAdminGroupRequiredAccountLevel(PlatformOpenAI, level)
			require.NoError(t, err)
