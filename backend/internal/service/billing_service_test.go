...
func TestCalculateCostWithServiceTier_PriorityAppliesPerComponentFallback(t *testing.T) {
	svc := newTestBillingService()
	...
	require.InDelta(t, 0.5, cost.CacheCreationCost, 1e-12)
	require.InDelta(t, 0.05, cost.CacheReadCost, 1e-12)
	require.InDelta(t, 4.55, cost.TotalCost, 1e-12)
}
...
func TestGetModelPricingWithChannel_OverrideInputPriceOnly(t *testing.T) {
	...
	// Channel prices own both normal and priority tiers.
...
func TestComputeTokenBreakdown_ExplicitZeroImagePriceNoFallback(t *testing.T) {
	svc := newTestBillingService()
	pricing := &ModelPricing{
		InputPricePerToken:       3e-6,
		OutputPricePerToken:      15e-6,
...
func TestComputeTokenBreakdown_NonExplicitZeroImagePriceFallsBackToOutput(t *testing.T) {
	svc := newTestBillingService()
	pricing := &ModelPricing{
		InputPricePerToken:       3e-6,
		OutputPricePerToken:      15e-6,
