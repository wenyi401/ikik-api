...
// SelectAccountForTokenCount applies normal eligibility checks for a
// non-billable token-count request without acquiring a generation slot.
func (s *OpenAIGatewayService) SelectAccountForTokenCount(
	...
	return s.selectAccountForModelWithExclusions(
		ctx, groupID, platform, sessionHash, requestedModel, nil, false, 0,
...
func NormalizeOpenAICompatiblePlatform(platform string) string {
	switch platform {
	case PlatformGrok, PlatformKiro, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo:
		return platform
	default:
...
// openAICompatibleAccountEligibilityFailureReason mirrors the eligibility
// check while naming its first veto point for server-side diagnostics.
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	...
	// Under a profit gate, binding is deferred until terminal admission so a
	// late-vetoed account never becomes the new sticky target.
