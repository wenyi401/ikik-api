...
func TestApplyCodexOAuthTransform_SkipDefaultInstructionsLeavesEmptyInstructions(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.1",
	}

	_ = applyCodexOAuthTransformWithOptions(reqBody, codexOAuthTransformOptions{
		IsCodexCLI:              true,
	...
	require.True(t, ensureCodexOAuthInstructionsField(reqBody))
...
func TestApplyCodexOAuthTransform_GPT55SuppliesModelSpecificInstructions(t *testing.T) {
	...
	instructions, ok := reqBody["instructions"].(string)
	require.True(t, ok)
	require.Contains(t, instructions, "You are GPT-5.5 running in the Codex CLI")
