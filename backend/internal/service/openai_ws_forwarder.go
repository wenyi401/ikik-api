...
func (s *OpenAIGatewayService) SelectAccountByPreviousResponseID(
	...
	if s.shouldClearStickySessionForSchedulingRequest(ctx, account, requestedModel) ||
		!s.isOpenAIAccountEligibleForSchedulingRequest(ctx, account, PlatformOpenAI, requestedModel, requireCompact, "") {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return nil, nil
	}
	...
	account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, PlatformOpenAI, requestedModel, requireCompact, "")
	if account == nil {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return nil, nil
	}
...
func (s *OpenAIGatewayService) persistOpenAIWSRateLimitSignal(ctx context.Context, account *Account, headers http.Header, responseBody []byte, codeRaw, errTypeRaw, msgRaw string) {
	...
	if isOpenAIModelCapacityError(http.StatusServiceUnavailable, strings.TrimSpace(msgRaw+" "+codeRaw+" "+errTypeRaw), responseBody) {
		s.handleOpenAIAccountUpstreamError(ctx, account, http.StatusServiceUnavailable, headers, responseBody)
		return
	}
