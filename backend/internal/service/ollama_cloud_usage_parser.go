...
func parseOllamaCloudUsageHTML(body []byte) (*OllamaCloudUsageData, error) {
	...
	if ollamaContainsAny(lowerPage, "sign in to ollama", "log in to ollama", "continue to sign in") {
...
func ollamaModelWindow(node *html.Node) OllamaCloudUsageModelWindow {
	for block := parentNode(node); block != nil; block = block.Parent {
		if value, ok := htmlAttribute(block, "data-usage-window"); ok {
			...
			if ollamaContainsAny(normalized, "five hour", "5 hour", "5h", "session") {
				return OllamaCloudUsageModelWindowFiveHour
			}
			if ollamaContainsAny(normalized, "seven day", "7 day", "7d", "weekly") {
				return OllamaCloudUsageModelWindowSevenDay
			}
		}
		text := strings.ToLower(normalizedNodeText(block))
		fiveHour := ollamaContainsAny(text, ollamaFiveHourUsageAliases...)
		sevenDay := ollamaContainsAny(text, ollamaSevenDayUsageAliases...)
...
func ollamaContainsAny(value string, candidates ...string) bool {
