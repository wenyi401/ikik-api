...
func TestNormalizeCodexModel(t *testing.T) {
	cases := map[string]string{
		...
		"gpt-5.6-terra-custom":      "gpt-5.6-terra-custom",
		"gpt-5.6-luna-2026-08-15":   "gpt-5.6-luna",
...
func TestNormalizeOpenAIModelForUpstream(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "oauth preserves explicit GPT-5.6 Sol channel alias",
			account: &Account{Type: AccountTypeOAuth},
			model:   "gpt-5.6-sol-wm",
			want:    "gpt-5.6-sol-wm",
		},
		{
			name:    "oauth still normalizes known GPT-5.6 Sol effort suffix",
			account: &Account{Type: AccountTypeOAuth},
