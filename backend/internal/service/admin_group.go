...
func (s *adminServiceImpl) UpdateCompositeRoute(ctx context.Context, groupID, routeID int64, input CompositeRouteInput) (*CompositeModelRoute, error) {
	...
	ok, err := s.compositeRouteBelongsToGroup(ctx, groupID, routeID)
	if err != nil {
		return nil, err
	}
	if !ok {
...
func groupSupportsOAuthOnlyFilter(platform string) bool {
	return platform == PlatformOpenAI || platform == PlatformAntigravity ||
...
func normalizeAdminGroupRequiredAccountLevel(platform, level string) (string, error) {
	level = strings.ToLower(strings.TrimSpace(level))
	if !IsValidRequiredAccountLevel(level) {
		return "", fmt.Errorf("invalid required_account_level: %s", level)
	}
	if platform != PlatformOpenAI {
		if level != "" {
			return "", errors.New("required_account_level is only supported for openai groups")
		}
		return "", nil
	}
	return NormalizeRequiredAccountLevel(level), nil
}
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	requiredAccountLevel, err := normalizeAdminGroupRequiredAccountLevel(platform, input.RequiredAccountLevel)
	if err != nil {
		return nil, err
	}
	isSharedPool := input.IsSharedPool
	if platform == PlatformOpenAI && requiredAccountLevel != "" {
		isSharedPool = true
	}
	if isSharedPool && !supportsOwnedPublicSharePoolPlatform(platform) {
		return nil, fmt.Errorf("shared account pools are not supported for platform %s", platform)
	}
	...
	profitControlEnabled, profitMinMargin, profitSafetyBuffer := NormalizeProfitControlConfig(
		platform, input.ProfitControlEnabled, profitMinMargin, profitSafetyBuffer,
	)
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.RequiredAccountLevel != nil {
		requiredAccountLevel, err := normalizeAdminGroupRequiredAccountLevel(group.Platform, *input.RequiredAccountLevel)
		if err != nil {
			return nil, err
		}
	...
	if group.Platform == PlatformOpenAI && NormalizeOpenAISharedPoolRequiredLevel(group.RequiredAccountLevel) != "" {
		group.IsSharedPool = true
	}
	if group.IsSharedPool && !supportsOwnedPublicSharePoolPlatform(group.Platform) {
		return nil, fmt.Errorf("shared account pools are not supported for platform %s", group.Platform)
	}
	if input.Status != "" {
	...
	group.ProfitControlEnabled, group.ProfitMinMargin, group.ProfitSafetyBuffer = NormalizeProfitControlConfig(
		group.Platform, group.ProfitControlEnabled, group.ProfitMinMargin, group.ProfitSafetyBuffer,
	)
