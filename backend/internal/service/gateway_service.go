...
// platformUserID: 系统用户 ID，用于二维亲和调度
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, platformUserID int64) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	...
	carpoolPlatforms := []string{platform}
	if useMixed {
		carpoolPlatforms = []string{platform, PlatformAntigravity}
	}
	if accounts, isCarpool, err := listCarpoolSchedulingAccounts(ctx, s.carpoolRepo, s.accountRepo, groupID, carpoolPlatforms); err != nil {
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, _, err := s.buildUpstreamRequestAnthropicAPIKeyPassthrough(upstreamCtx, c, account, input.Body, token)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
...
func (s *GatewayService) parseSSEUsagePassthrough(data string, usage *ClaudeUsage) {
	...
	case "message_delta":
		...
		if deltaUsage.Exists() {
			...
			if v := extractReasoningTokensFromUsageNode(deltaUsage); v > 0 {
...
func parseClaudeUsageFromResponseBody(body []byte) *ClaudeUsage {
	...
	usage.ReasoningTokens = extractReasoningTokensFromUsageNode(usageNode)
...
func (s *GatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*ClaudeUsage, error) {
	...
	response.Usage.ReasoningTokens = extractReasoningTokensFromJSONBytes(body)
...
func finalizePostUsageBilling(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill {
		...
		if result != nil && result.CommissionDeducted > 0 && p.User != nil {
			syncBalanceCacheAfterDeduction(p, deps, result, result.CommissionDeducted)
		}
		...
		if result != nil && result.PointsDeducted > 0 {
			...
			syncBalanceCacheAfterDeduction(p, deps, result, result.BalanceDeducted)
		} else {
			syncBalanceCacheAfterDeduction(p, deps, result, p.Cost.ActualCost)
		}
	}
...
func syncBalanceCacheAfterDeduction(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult, amount float64) {
	if p == nil || p.User == nil || deps == nil || deps.billingCacheService == nil || amount <= 0 {
		return
	}
	if result != nil && result.NewBalance != nil && deps.billingCacheService.balanceBelowEligibilityThreshold(*result.NewBalance) {
		if err := deps.billingCacheService.InvalidateUserBalance(context.Background(), p.User.ID); err != nil {
...
// ApplyBedrockCCCompat keeps the handler-level body mutation hook available while
// leaving the canonical Bedrock preparation in forwardBedrock.
func (s *GatewayService) ApplyBedrockCCCompat(ctx context.Context, body []byte, modelID string, account *Account, groupID *int64) []byte {
	_ = groupID
	if account == nil || !account.IsBedrock() {
		return body
	}
	betaTokens, err := s.resolveBedrockBetaTokensForRequest(ctx, account, "", body, modelID)
	if err != nil {
		return body
	}
	bedrockBody, err := PrepareBedrockRequestBodyWithTokens(body, modelID, betaTokens)
	if err != nil {
		return body
	}
	return bedrockBody
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if filtered := StripEmptyTextBlocks(body); !bytes.Equal(filtered, body) {
		if err := replaceBody(filtered); err != nil {
			s.countTokensError(c, http.StatusBadRequest, "invalid_request_error", "Failed to normalize request")
			return err
		}
	}
