...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if result != nil && deps.billingCacheService != nil {
		for _, userID := range result.BalanceCreditUserIDs {
			...
			if err := deps.billingCacheService.InvalidateUserBalance(ctx, userID); err != nil {
				slog.Warn("invalidate balance cache after account share credit failed",
					"user_id", userID,
					"error", err,
				)
			}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	if billingErr != nil {
		// Preserve the observed upstream usage for audit and reconciliation, but
		// record it as unsettled so it cannot be interpreted as a completed debit.
