...
func TestBuildOpenAIImagesResponsesRequest_PassesMultipleImageCount(t *testing.T) {
...
func TestBuildOpenAIImagesResponsesRequest_PassesInputFidelity(t *testing.T) {
	...
	body, err := buildOpenAIImagesResponsesRequest(parsed, "gpt-image-2", OpenAIImagesResponsesReasoningEffortHigh)
	require.NoError(t, err)
	require.NotNil(t, body)
...
func TestExtractOpenAIImagesModelRefusalFromResponsesSSE(t *testing.T) {
	body := []byte(strings.Join([]string{
		`data: {"type":"response.output_text.delta","delta":"I can't help "}`,
		`data: {"type":"response.output_text.delta","delta":"create that image."}`,
		`data: {"type":"response.completed","response":{"output":[{"type":"message","content":[{"type":"output_text","text":"Please try a safer prompt."}]}]}}`,
		`data: [DONE]`,
	}, "\n\n"))

	refusal := extractOpenAIImagesModelRefusal(body)
	require.Contains(t, refusal, "I can't help")
	require.Contains(t, refusal, "create that image")
	require.Contains(t, refusal, "Please try a safer prompt")
}
