...
// The normal Chat Completions 鈫?Responses conversion path is unaffected
// because ChatCompletionsRequest has no fields for these parameters 鈥?unknown
...
// ForwardAsChatCompletions accepts a Chat Completions request body. API key
// accounts whose upstream does not expose /v1/responses are forwarded directly
// to /v1/chat/completions; other accounts keep the Responses conversion path.
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	// API shaped bodies 鈥?`input: [...]` with no `messages` field 鈥?to the
	...
	if isResponsesShape {
		...
		// we must filter these fields explicitly here 鈥?otherwise the upstream
		...
		// Normal path: convert Chat Completions 鈫?Responses.
	...
	if account.Type == AccountTypeOAuth {
		...
		if !isResponsesShape && ensureCodexOAuthInstructionsField(reqBody) {
