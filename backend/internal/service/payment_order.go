...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, limitAmount float64, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan, sel *payment.InstanceSelection) (*CreateOrderResponse, error) {
	...
	subject := s.buildPaymentSubject(plan, limitAmount, cfg, sel, req)
...
func (s *PaymentService) buildPaymentSubject(plan *dbent.SubscriptionPlan, limitAmount float64, cfg *PaymentConfig, sel *payment.InstanceSelection, requests ...CreateOrderRequest) string {
	if plan != nil {
		productName := plan.ProductName
		if productName == "" {
			productName = "ikik-api Subscription " + plan.Name
		}
		return applyPaymentProductNameAffix(productName, cfg)
	}
	if len(requests) > 0 && requests[0].OrderType == payment.OrderTypeShop {
		if subject := strings.TrimSpace(requests[0].Subject); subject != "" {
			return subject
		}
