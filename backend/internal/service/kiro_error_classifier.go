...
func isKiroModelAccessError(statusCode int, upstreamMsg string, responseBody []byte) bool {
	...
	raw := strings.ToLower(strings.Join([]string{
		upstreamMsg,
		extractUpstreamErrorCode(responseBody),
		...
		gjson.GetBytes(responseBody, "reason").String(),
		gjson.GetBytes(responseBody, "message").String(),
		body,
	}, " "))
	collapsed := strings.Join(strings.Fields(strings.NewReplacer("_", " ", "-", " ").Replace(raw)), " ")

	if looksLikeKiroBadRequestInvalidModelError(raw) ||
		strings.Contains(collapsed, "invalid model") ||
		strings.Contains(collapsed, "unsupported model") ||
		strings.Contains(collapsed, "model not supported") ||
		strings.Contains(collapsed, "not supported model") ||
		strings.Contains(collapsed, "no access to model") {
		return true
	}

	if strings.Contains(raw, "no_access") && strings.Contains(collapsed, "model") {
		return true
	}

	return strings.Contains(collapsed, "model") &&
		(strings.Contains(collapsed, "not available") ||
			strings.Contains(collapsed, "not allowed") ||
			strings.Contains(collapsed, "permission denied") ||
			strings.Contains(collapsed, "lack permissions"))
}
