...
func TestSettingService_GetPublicSettings_ExposesIkikFeatureFlags(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
	...
	require.True(t, ok)
	require.True(t, payload.FreeModelsEnabled)
	require.True(t, payload.CarpoolEnabled)
}
